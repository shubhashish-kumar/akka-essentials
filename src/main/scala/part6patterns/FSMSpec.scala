...
import org.scalatest.{BeforeAndAfterAll, OneInstancePerTest, WordSpecLike, stats}
...
object FSMSpec {
  ...
  case class Instruction(msg: String) // message the VM will show on its screen.
  case class ReceiveMoney(amount: Int)
  case class DeliverProduct(product: String)
    ...
    def waitForMoney(
      ...
      case ReceiveMoney(amount) =>
        ...
        if(money + amount >= price) {
          ...
          requester ! DeliverProduct(product)
          //deliver the change
          ...
          context.become(waitForMoney(
            ...
            startReceiveMoneyTimeoutSchedule, // I need to set the timer again
            requester))
        }
    }
