...
object AskSpec {
  ...
  class KVActor extends Actor with ActorLogging {
    ...
    def online(kv: Map[String, String]): Receive = {
      case Read(key) =>
        log.info(s"Trying to read the value for the key $key from the map") //Option
        sender() ! kv.get(key)
      case Write(key, value) =>
        log.info(s"Writing key $key and value $value pair into the map")
  ...
  object AuthManager {
    val AUTH_FAILURE_NOT_FOUND = "usenname not found"
